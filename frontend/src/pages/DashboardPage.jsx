...
function StatCard({ label, value, sub, icon: Icon, color = 'bg-neutral-50' }) {
  return (
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      {quota && user?.plan !== 'UNLIMITED' && (
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            {user.plan === 'FREE' && (
              <Link to="/pricing" className="btn-primary text-xs px-3 py-2">
                <Zap size={12} /> Upgrade
              </Link>
            )}
          </div>
          <div className="h-2 bg-neutral-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${quota.pct >= 90 ? 'bg-red-500' : quota.pct >= 70 ? 'bg-amber-500' : 'bg-brand-500'}`}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard label="Total variants"  value={data?.stats.totalVariants ?? 0} icon={Zap}        sub="Generated variants" />
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-neutral-900">Recent uploads</h2>
          ...
          <div className="divide-y divide-neutral-50">
            {data.recentUploads.map((upload) => (
              <Link key={upload.id} to={`/upload/${upload.id}`}
                className="flex items-center gap-4 py-3 hover:bg-neutral-50 -mx-2 px-2 rounded-xl transition-colors group">
                <div className="w-12 h-12 rounded-xl bg-neutral-100 overflow-hidden flex-shrink-0">
                  {upload.topVariant?.url
                    ? <img src={upload.topVariant.url} alt="" className="w-full h-full object-cover" />
                    : <div className="w-full h-full flex items-center justify-center"><Package size={18} className="text-neutral-400" /></div>
                  }
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-neutral-900 truncate">{upload.originalName}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <Clock size={11} className="text-neutral-400" />
                    <span className="text-xs text-neutral-400">{new Date(upload.createdAt).toLocaleDateString('en-IN')}</span>
